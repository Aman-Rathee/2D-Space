...
export const Chat = ({ ws }: { ws: WebSocket }) => {
    ...
    return (
        <>
            <div className="bg-red-600 h-[calc(100vh-49px)] w-72 fixed top-0 right-0">
                    ...
                    <div className="flex-1 p-4 overflow-y-auto">
                        {messages.map((message, index) => (
                            <div key={index} className={`mb-2 ${message.user === "you" ? "text-right" : "text-left"}`}>
                                {message.user === 'you' ?
                                    <span className={"inline-block px-2 py-1 rounded-md bg-blue-500 text-white"}>
                                        {message.text}
                                    </span> :
                                    <span className={"inline-block px-2 py-1 rounded-md bg-gray-300 text-gray-700"}>
